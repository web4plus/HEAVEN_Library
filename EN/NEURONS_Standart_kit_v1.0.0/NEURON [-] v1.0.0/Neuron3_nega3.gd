...
func CONNECT_DATA():
	...
	$Activation_Level.text = "AL: "+str(NodeData["Params"]["Activation_level"])
	# / БЛОК ИНТЕГРАЦИИ С ПОЛЕМ ИНФОРМАЦИИ
...
#				# Если нода существует
				if receiver_node != null:
					...
					get_parent().get_node(NEIGH).DECR(NodeData["Params"]["Activation_level"]) 				
					$ColorRect.color = Color(255,255,255)
...
func _on_EXHIBITION_close_request():
