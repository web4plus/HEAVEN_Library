...
# !! Данная переменная будет содержать блок данных содержащихся в NSA
var NSA_DATABLOCK = null
...
# Тип нейрона
var node_type = "NSA" # ХВАТИТ ЛИ УКАЗАНИЯ В КОРНЕ?
...
func CONNECT_DATA():
	...
	# Но если запись о этой ноде не обнаружена в поле данных
		...
		GraphData[name]["Params"]["Potential"] = potential
		GraphData[name]["Params"]["Activation_level"] = activation_level
		GraphData[name]["Params"]["Datablock"] = $datablock_input.text
...
func SUMM(input_value):
	...
	# Сюда вставлен кусок обращения к последующему нейрону, таким образом сигнал передается в не зависимости от порога активации, сразу
...
#				# Если нода существует
			if receiver_node != null:
				...
				get_parent().get_node(NEIGH).INIT(NodeData["Connections"][0][NEIGH]['SLOT_RECVR'], GraphData[name]["Params"]["Datablock"])#NSA_DATABLOCK)               #.SUMM(NodeData["Params"]["Activation_level"]) 				
...
#				# Если нода существует
				if receiver_node != null:
					...
					get_parent().get_node(NEIGH).INIT(0, GraphData[name]["Params"]["Datablock"])               #.SUMM(NodeData["Params"]["Activation_level"]) 				
					$ColorRect.color = Color(255,255,255)
					NodeData["Params"]["Indicator"] = 1.0
...
func _on_CURRENT_pressed():
	print(potential)
	#31012025 Error on value changer ( given array )
...
func _on_LineEdit_text_changed(new_text):
	GraphData[name]["Params"]["Datablock"] = $datablock_input.text
	$Current_datablock.text = GraphData[name]["Params"]["Datablock"] 
	print('changed')
	pass # Replace with function body.
